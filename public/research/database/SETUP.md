...
1. เปิดไฟล์ `/database/sql/account-org-schema.sql`
...
1. เปิดไฟล์ `/database/sql/role-permission-schema.sql`
2. Copy ทั้งหมด
...
1. เปิดไฟล์ `/database/sql/role-permission-seed-data.sql`
2. Copy ทั้งหมด
3. Paste ลงใน SQL Editor (New Query)
4. คลิก **Run**
5. รอให้ script รันเสร็จ
...
### Master Data ไม่ครบ
**Solution**: รัน INSERT statements ใน `/database/sql/account-org-schema.sql` อีกครั้ง:
```sql
