"use client";

import React, { useEffect } from "react";
import { useRouter } from "next/navigation";
import { SellerAppShell } from "@/components/seller/layout/SellerAppShell";
import { SellerHeader } from "@/components/seller/SellerHeader";
import { useSellerSession } from "@/lib/hooks/useSellerSession";
import { signOut } from "@/lib/supabase/auth";
import { message, Spin } from "antd";

export default function SellerLayout({ children }: { children: React.ReactNode }) {
    const router = useRouter();
    const { user, org, organizations, memberships, shop, branches, loading, switchOrg, switchShop } = useSellerSession();

    const handleLogout = async () => {
        await signOut();
        message.success("ออกจากระบบเรียบร้อยแล้ว");
        router.push("/seller/login");
    };

    useEffect(() => {
        if (!loading && !user) {
            router.push("/seller/login");
        }
    }, [user, loading, router]);

    if (loading) {
        return (
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>
                <Spin size="large">
                    <div style={{ marginTop: 44 }}>กำลังเตรียมข้อมูลร้านค้า...</div>
                </Spin>
            </div>
        );
    }

    if (!user) return null;

    // Prepare info for header according to real data
    const userInfo = user ? {
        id: user.id,
        name: user.displayName || `${user.firstName} ${user.lastName}`,
        email: user.email,
        avatar: user.avatarUrl,
        isVerified: user.emailVerified && user.phoneVerified,
    } : undefined;

    const shopInfo = shop ? {
        id: shop.id,
        name: shop.name,
        subdomain: shop.subdomain,
        logo: shop.logoUrl,
        type: shop.type as any,
        isActive: shop.isActive,
        orgId: shop.orgId,
    } : undefined;

    const orgInfo = org ? {
        id: org.id,
        name: org.name,
        logo: org.logoUrl,
        type: org.type as any,
        role: memberships?.find((m: any) => m.orgId === org.id)?.role === 'owner' ? "เจ้าของ" : "สมาชิก",
        kybVerified: org.kybStatus === "verified",
    } : undefined;

    const orgsList = (organizations || []).map((o: any) => ({
        id: o.id,
        name: o.name,
        logo: o.logoUrl,
        type: o.type as any,
        role: memberships?.find((m: any) => m.orgId === o.id)?.role === 'owner' ? "เจ้าของ" : "สมาชิก",
        kybVerified: o.kybStatus === "verified",
    }));

    // Map list of shops/branches for selection
    const allShopsList = shop ? [shopInfo] : [];
    const branchesList = branches.map((b: any) => ({
        id: b.id,
        name: b.name,
        logo: undefined,
        type: "branch" as const,
        isActive: b.isActive,
        orgId: org?.id || "",
    }));

    return (
        <SellerAppShell
            headerSlot={
                <SellerHeader
                    currentShop={shopInfo}
                    shops={[...(allShopsList as any), ...(branchesList as any)]}
                    currentOrg={orgInfo}
                    orgs={orgsList}
                    user={userInfo}
                    notificationCount={5}
                    onShopChange={switchShop}
                    onOrgChange={switchOrg}
                    onProfile={() => router.push("/profile")}
                    onSettings={() => router.push("/shop-management")}
                    onManageOrgs={() => router.push("/organization-settings")}
                    onNotificationClick={() => message.info("Mock Notifications")}
                    onLogout={handleLogout}
                    onMobileMenuClick={() => console.log("Mobile menu clicked")}
                />
            }
        >
            {children}
        </SellerAppShell>
    );
}
