...
- **Buyers**: Legal Entity (Company) only
  - Individual Consumer: ไม่ต้อง KYC (MVP)
  - Registered Individual Merchant: อาจต้อง KYC (ถ้าต้องการ B2B features)

**KYC Exemptions**:
- Individual Consumer (B2C buyers) - ไม่ต้อง KYC สำหรับ MVP
...
**Required KYB**:
- **All ORDs**: ทุกองค์กรที่สร้าง ORD
  - Seller ORD (ต้องมี Shop)
  - Buyer ORD (Legal Entity)

**KYB Purpose**:
- Verify business legitimacy
- Enable ORD features (Shop creation, B2B features)
...
**Step 1: Create ORD**
1. Account creates ORD
2. System prompts for KYB
3. User cannot proceed without KYB
...
**Step 4: Approval/Rejection**
1. Admin approves or rejects
2. If approved → KYB Status: Approved (ORD Verified)
3. If rejected → KYB Status: Rejected (with reason)
4. User notified of result

**Step 5: ORD Activation**
1. If KYB approved → ORD Verified
2. Seller can create Shop
3. Buyer can use B2B features
4. If KYB rejected → User can resubmit
...
**Status Effects**:
- **Approved**: ORD Verified → Can use all features
- **Rejected**: Cannot use ORD features
- **Expired**: Must re-verify to continue
- **Suspended**: ORD temporarily disabled
...
**US-KYB-001: Complete KYB for ORD**
- **As a** user creating ORD
- **I want to** complete KYB
- **So that** my ORD can be verified
- **Acceptance Criteria**:
  - Can upload KYB documents
  - Can see verification status
  - Receive notification when approved/rejected
  - ORD activated when approved

**US-KYB-002: Re-verify KYB**
- **As a** user with expired KYB
- **I want to** re-verify KYB
- **So that** I can continue using ORD features
- **Acceptance Criteria**:
