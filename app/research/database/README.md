...
### 1. [account-org-schema.sql](./account-org-schema.sql) ✅ **MAIN SCHEMA**
...
- Organization (ORD) table (ตาม schema specification)
...
### 2. [role-permission-schema.sql](./role-permission-schema.sql) ✅ **NEW**
...
### 3. [role-permission-seed-data.sql](./role-permission-seed-data.sql) ✅ **NEW**
...
- **Organization Table**: ORD (KYB status) - นิติบุคคล
- **Enums**: Customer Status, Organize Type, KYC Status
- **Reference Tables**: HighestAuthority, Contact
- **ERD Tables**: user_registration, user_attributes, user_preferences, kyc, user_organizations
...
- `organizations` - ORD (KYB status) - นิติบุคคล
...
2. **Step 1**: Copy content from `account-org-schema.sql` and run
3. **Step 2**: Copy content from `role-permission-schema.sql` and run
4. **Step 3**: Copy content from `role-permission-seed-data.sql` and run
...
### Immediate Actions
1. ✅ Review `account-org-schema.sql`
...
2. **Schema Order**:
   - Step 1: `account-org-schema.sql` (Account & ORG)
   - Step 2: `role-permission-schema.sql` (Role & Permission)
   - Step 3: `role-permission-seed-data.sql` (Seed Data & Functions)
