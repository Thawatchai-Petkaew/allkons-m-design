...
- **Structure**: Account (KYC) → ไม่มี ORD
- **Permissions**: 
  - ซื้อสินค้าได้
  - ดูราคาแบบ B2C
...
- **KYC**: ต้อง KYC
- **Structure**: Account (KYC) → ไม่มี ORD (หรือมี ORD ถ้าต้องการ)
- **Permissions**:
  - ซื้อสินค้าได้
...
- **KYC**: ต้อง KYC
- **Structure**: Account (KYC) → ORD (KYB) → Team Members
  - **Note**: Buyer ไม่มี Shop และ Branch
- **Permissions**:
  - สร้างหลาย ORD ได้ (1 Account → หลาย ORD)
  - ซื้อสินค้าได้
  - ดูราคาแบบ B2B (ถ้าได้รับการอนุมัติ)
  - ใช้ฟีเจอร์ B2B เต็มรูปแบบ (PO, credit terms, invoice)
  - ไม่มีสิทธิ์ขายสินค้า (ถ้าไม่ได้สมัครเป็น seller)
  - **Organization Management**: จัดการ ORD, Team Members, Role & Permissions, Financial
  - **Team Management**: เพิ่มสมาชิกทีมเข้า ORD เพื่อช่วยจัดการคำสั่งซื้อ
- **Personalization**:
...
- **KYC**: ต้อง KYC
- **Structure**: Account (KYC) → ORD (KYB) → Shop → Branch
- **Permissions**:
  - สร้าง ORD (ต้องผ่าน KYB)
  - สร้าง Shop (1 Shop ต่อ 1 ORD)
  ...
  - จัดการออเดอร์
  - ดูรายงานการขาย
  - **Organization Management**: จัดการ ORD, Team Members, Role & Permissions
...
- **KYC**: ต้อง KYC
- **Structure**: Account (KYC) → ORD (KYB) → Shop → Branch
  - สามารถสร้างหลาย ORD ได้ (1 Account → หลาย ORD)
  - แต่ละ ORD สร้าง Shop ได้ 1 Shop
...
- **Permissions**:
  ...
  - **Organization Management**: จัดการ ORD, Team Members, Role & Permissions, Financial
  - **Team Management**: เพิ่มสมาชิกทีมเข้า ORD และ Assign เข้า Shop
...
| Context | User Type | KYC Required | ORD Required | Shop Required | Branch Required |
|---------|-----------|--------------|--------------|---------------|-----------------|
| **Buyer** | บุคคลธรรมดา | ❌ | ❌ | ❌ | ❌ |
...
| **Team Members** | ✅ มี (ใน ORD) | ✅ มี (ใน ORD + Shop) |
...
```
บุคคลธรรมดา:
Account (KYC) → (ไม่มี ORD)

บุคคลธรรมดาจดทะเบียนพาณิชย์:
Account (KYC) → (ORD Optional)

นิติบุคคล:
Account (KYC) → ORD (KYB) → Team Members
```
...
```
บุคคลธรรมดาจดทะเบียนพาณิชย์:
Account (KYC) → ORD (KYB) → Shop → Branch (Optional)

นิติบุคคล:
Account (KYC) → ORD (KYB) → Shop → Branch (Optional)
  └── (สามารถมีหลาย ORD)
      └── แต่ละ ORD → Shop → Branch
```
...
3. **นิติบุคคล**:
   - ลงทะเบียน → สร้าง ORD → อัปโหลดเอกสารบริษัท → KYC → KYB → ใช้งานได้

### Seller Registration

1. **บุคคลธรรมดาจดทะเบียนพาณิชย์**:
   - ลงทะเบียน → อัปโหลดเอกสารพาณิชย์ → KYC → สร้าง ORD → อัปโหลดเอกสาร → KYB → สร้าง Shop → ใช้งานได้

2. **นิติบุคคล**:
   - ลงทะเบียน → สร้าง ORD → อัปโหลดเอกสารบริษัท → KYC → KYB → สร้าง Shop → ใช้งานได้
